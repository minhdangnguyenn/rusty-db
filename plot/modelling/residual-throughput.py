...
def _summary_files(label: str, size: str, dist: str, cached: bool):
    base = CACHE_DIR if cached else NOCACHE_DIR
    pattern = (
        f"exp3-{label}-{size}-{dist}-*-summary.csv"
        if cached
        else f"exp3-nocache-{label}-{size}-{dist}-*-summary.csv"
    )
    return sorted(glob.glob(os.path.join(base, label, size, dist, "**", pattern),
                            recursive=True))
...
def main():
    ...
    fig.suptitle("Measured / M/M/m throughput ratio (log scale)", fontsize=13,
                 y=0.995)
    ...
    for ax, (size, dist) in zip(axes.flat, combos):
        ...
        ratio_cache = [(m / p) if (m and p) else None
                       for m, p in zip(meas_cache, mmm_pred)]
        ratio_nc = [(m / p) if (m and p) else None
        ...
        ax.set_xticklabels([f"c{k}" for k in K_VALUES])
        ax.set_xlim(3, 80)
        ax.set_yscale("log")
    ...
    for ax in axes[:, 0]:
        ax.set_ylabel("measured / M/M/m  (log scale)")
